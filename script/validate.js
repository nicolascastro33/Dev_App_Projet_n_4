...
export function validateSubmitForm(input, allInformation){
     if(input.type === "text"){
       let nomOuPrénom = input.name === "last" ? "nom" : "prénom";
       allInformation[`${nomOuPrénom}`] = validateName(input.value, nomOuPrénom, input);
     }else if(input.type === "email"){
       allInformation.email = validateEmail(input.value, input);
     }else if(input.type === "date"){
       allInformation.birthDate = validateBirthDate(input.value, input);
     }else if(input.type === "number"){
       allInformation.numberOfTournamentPlayed = validateNumberOfTournement(input.value, input);
     }else if(input.name === "location"){
       allInformation.location = validateLocationTournament(getInputLocation(allInputForm), input);
